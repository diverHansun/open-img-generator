...
2. 影响行数 0：另一请求正在租约期内推进，当前请求跳过。进程异常后租约（300 秒）到期，下一次 GET 可恢复。
3. 结果写回使用 claim 时的 `poll_lease_until` 作为 CAS 条件；租约过期后重新 claim 的 worker 不会被旧请求覆盖。图片记录按 `(generation_job_id, index)` 幂等落库，竞争失败的临时文件会清理。
4. poll 结果、解析失败、provider 不可用和转存结束都会清空租约；转存通过 `(generation_job_id, index)` 唯一键幂等。
