...
| 轮询排他 | `pollLeaseUntil`（建议 35 秒）独立字段 | 保留真实 `pending/running` 状态，崩溃后可恢复 | lease 到期前最多延迟一次 poll；不提供后台自动推进 |
...
| 风险 | 防护 | 回滚 |
|---|---|---|
| 租约过短导致重复 poll | 35 秒超过单次 Fal 双请求超时预算；原子条件写入 | 增加租约常量，不改 status 语义 |
