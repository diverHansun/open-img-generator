...
- `IMAGE_RETENTION_DAYS=7`（设为 `0` 禁用）把过期且未收藏图片转为 `retention_expired` 墓碑并删除应用内字节；Generation、Job、Prompt、Provider error 与图片元数据继续保留。孤儿文件需超过 `IMAGE_ORPHAN_GRACE_MS` 才删除，收藏不会因保留期清理。
