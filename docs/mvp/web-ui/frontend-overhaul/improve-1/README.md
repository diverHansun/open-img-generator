...
> 状态：规划文档已完成，待整体审阅<br>
> 日期：2026-07-18（基于第二轮讨论与本轮逐页收敛，决策记录见 `00-discussion.md` §0.9）<br>
> 文档落点：`docs/mvp/web-ui/frontend-overhaul/improve-1/`<br>
> 实施状态：**未开始；本目录定义后续实施契约，不包含代码改动**
...
8. 视觉层级统一、减少无意义卡片与阴影，同时保留当前暖白与陶土色的创作工具气质。
...
根级 `02` 与 `04` 是后续实施会话的执行契约；本规划会话不按 `02` 修改代码。
...
1. 根级 `00`–`04`、`shared/` 与七个页面文档集已完成，并经第二轮讨论修订（见 `00-discussion.md` §0.9）。
2. 文档内部已经执行范围、术语、API 所有权和验收映射检查。
3. 用户整体审阅并批准后，规划阶段结束。
4. 后续实施会话先按根级 `02` 的 Backend Contract → Frontend API Wiring 顺序执行；视觉实现须经过独立确认闸门后才开始，按根级 `04` 和页面级 `06` 验收。
5. 实施完成后再同步改写既有权威运行时文档；规划稿本身不宣称功能已经落地。
