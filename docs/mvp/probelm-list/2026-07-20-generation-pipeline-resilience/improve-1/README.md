...
| 批次 | 主题 | 独立提交与验证目标 |
|---|---|---|
| A | 数据库恢复与 readiness | 迁移旧库、schema preflight、健康契约与迁移回归测试 |
| B | 结构化错误与前端可行动反馈 | API error envelope、correlation ID、i18n 分类和提交错误测试 |
| C | 幂等接纳 | clientRequestId、payload hash、唯一约束与重复/并发提交测试 |
| D | 持久化 lifecycle | 请求/结果快照、dispatch/poll/store/cancel phase、状态单调、worker 与崩溃恢复 |
| E | Provider 与 storage 韧性 | 副作用感知重试、队列上限、远端图片安全、脱敏与副作用补偿 |
| F | 前端与端到端收口 | Generate/Stage 恢复、成本护栏、unit/integration/E2E/build、文档对齐和子代理复审 |
